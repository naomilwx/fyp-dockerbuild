...
sudo docker build -t development/dockerrosebuild .
...
if [ $OUT -ne 0 ]; then
	...
	sudo docker run -P -e --rm -v $CurrentPath/project:/root -v $CurrentPath/.ssh:/root/.ssh development/dockerrosebuild /bin/sh -c 'echo SUCCESS '
	exit 0
fi
